-- Script para exportar COMPLETAMENTE a estrutura do banco de dados (VERSÃO CORRIGIDA)
...
-- =============================================
-- 15. SCRIPT DE RECRIAÇÃO COMPLETA
-- =============================================

SELECT '=== SCRIPT DE RECRIAÇÃO COMPLETA ===' as section;

-- Gerar script de recriação para todas as tabelas
SELECT 
    '-- RECREATE TABLE: ' || table_schema || '.' || table_name as comment,
    'DROP TABLE IF EXISTS ' || table_schema || '.' || table_name || ' CASCADE;' as drop_statement
FROM information_schema.tables 
WHERE table_schema IN ('public', 'auth')
    AND table_type = 'BASE TABLE'
ORDER BY table_schema, table_name;
