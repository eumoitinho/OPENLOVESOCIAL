...
-- Based on the error, it seems the foreign key column name might be different
  ...
  FOR SELECT USING (
    ...
    (visibility = 'friends_only' AND (
      ...
      EXISTS (
        ...
        WHERE (user_id = posts.profile_id AND friend_id = auth.uid() AND status = 'accepted') OR
              (user_id = auth.uid() AND friend_id = posts.profile_id AND status = 'accepted')
      ) OR
      EXISTS (
